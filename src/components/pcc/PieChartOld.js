var DraggablePiechart = function (setup) {
  ...
  function touchMove(event) {
    ...
    var angleDegreesInt = Math.round(((180 / Math.PI) * newAngle) / 18) * 18;
    var adjustedAngle = degreesToRadians(angleDegreesInt);

    console.log(dx, dy, angleDegreesInt);

    piechart.shiftSelectedAngle(adjustedAngle);
    piechart.draw();

    if (angleDegreesInt % (360 / 20) === 0) {
    }
  }
...
DraggablePiechart.prototype.defaults = {
  ...
  drawSegment: function (
    ...
    var fontSize = Math.floor(context.canvas.height / 38);
